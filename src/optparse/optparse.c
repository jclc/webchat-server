...
#define MSG_INVALID "Error: Invalid option"
#define MSG_MISSING "Error: Option requires an argument"
#define MSG_TOOMANY "Error: Option takes no arguments"
...
opterror(struct optparse *options, const char *message, const char *data)
{
	...
	const char *append = "' (use -h for help)";
	while (*append)
		options->errmsg[p++] = *append++;
    options->errmsg[p++] = '\0';
    return '?';
}
